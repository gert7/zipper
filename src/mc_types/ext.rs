...
pub trait McAsyncReadExt: tokio::io::AsyncRead {
    async fn read_mc_byte(mut self: Pin<&mut Self>) -> tokio::io::Result<i8> {
        self.read_i8().await
    }

    async fn read_mc_ubyte(mut self: Pin<&mut Self>) -> tokio::io::Result<u8> {
        self.read_u8().await
    }

    async fn read_mc_short(mut self: Pin<&mut Self>) -> tokio::io::Result<i16> {
        // NOTE: reads are BIG ENDIAN
        self.read_i16().await
    }

    async fn read_mc_ushort(mut self: Pin<&mut Self>) -> tokio::io::Result<u16> {
        self.read_u16().await
    }

    async fn read_mc_int(mut self: Pin<&mut Self>) -> tokio::io::Result<i32> {
        self.read_i32().await
    }

    async fn read_mc_long(mut self: Pin<&mut Self>) -> tokio::io::Result<i64> {
        self.read_i64().await
    }
    
    async fn read_mc_float(mut self: Pin<&mut Self>) -> tokio::io::Result<f32> {
    ...
    async fn read_mc_double(mut self: Pin<&mut Self>) -> tokio::io::Result<f64> {
...
pub trait McAsyncWriteExt: tokio::io::AsyncWrite {
    async fn write_mc_byte(mut self: Pin<&mut Self>, value: i8) -> tokio::io::Result<()> {
        self.write_i8(value).await
    }

    async fn write_mc_ubyte(mut self: Pin<&mut Self>, value: u8) -> tokio::io::Result<()> {
        self.write_u8(value).await
    }

    async fn write_mc_short(mut self: Pin<&mut Self>, value: i16) -> tokio::io::Result<()> {
        // NOTE: reads are BIG ENDIAN
        self.write_i16(value).await
    }

    async fn write_mc_ushort(mut self: Pin<&mut Self>, value: u16) -> tokio::io::Result<()> {
        self.write_u16(value).await
    }

    async fn write_mc_int(mut self: Pin<&mut Self>, value: i32) -> tokio::io::Result<()> {
        self.write_i32(value).await
    }

    async fn write_mc_long(mut self: Pin<&mut Self>, value: i64) -> tokio::io::Result<()> {
        self.write_i64(value).await
    }
    
    async fn write_mc_float(mut self: Pin<&mut Self>, value: f32) -> tokio::io::Result<()> {
        let fake = unsafe {
    ...
    async fn write_mc_double(mut self: Pin<&mut Self>, value: f64) -> tokio::io::Result<()> {
        let fake = unsafe {
            std::mem::transmute(value)
        };
